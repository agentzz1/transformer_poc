"""Orbit Wars v9 — v5 focus-drip + neighbor rescue of threatened planets.
...
def agent(obs):
    ...
    # ---- phase 0: rescue threatened planets (deficit = enemy inbound - garrison) ----
    for m in mine:
        threat = enemy_in(m["id"])
        deficit = threat - (m["ships"] + friendly_in(m["id"])) + 2
        if deficit <= 0:
            continue
        helpers = sorted([h for h in mine if h["id"] != m["id"] and avail[h["id"]] >= 3],
                         key=lambda h: math.hypot(h["x"] - m["x"], h["y"] - m["y"]))
        for h in helpers:
            if deficit <= 0:
                break
            contrib = min(avail[h["id"]], int(math.ceil(deficit)))
            if contrib < 3:
                continue
            ax, ay = m["x"], m["y"]; eta = 1.0
            for _ in range(3):
                dist = math.hypot(ax - h["x"], ay - h["y"])
                eta = max(1.0, dist / _speed(contrib))
                ax, ay = predict(m, eta)
            if _seg_hits_sun(h["x"], h["y"], ax, ay) or eta > 25:
                continue
