"""Orbit Wars v8 — v5 focus-drip + ship-economy tuning mined from the #1 bot.

The top-rated bot wins with FEWER planets: it grabs high-production planets
early, then stops expanding and hoards (score = total ships; every capture
burns ships equal to the garrison). Changes vs v5: production-weighted target
value, 1.4x amortization margin, stricter ROI gate on attacking enemies.
...
def agent(obs):
    ...
    for src in sorted(mine, key=lambda m: front_d[m["id"]]):
        ...
        for tgt in others:
            ...
            if tgt["id"] in comet_ids:
                pay *= 0.35
            if pay < 1.4 * rem_need:  # demand clear profit, not break-even
                continue
            val = (tgt["prod"] ** 1.3 + 0.3) / (rem_need * (1.0 + 0.05 * eta))
            if val > best_val:
    ...
    for tgt in enemy_t:
        # capture gains us prod and denies it to the enemy (2x), but costs ~garrison
        if 2.0 * tgt["prod"] * max(0.0, remaining - 20) < 1.4 * (tgt["ships"] + 20):
            continue
