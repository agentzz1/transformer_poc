"""Orbit Wars v10b — multi-target drip per planet per turn, plus v10: v5 + three field-bug fixes (mined from losing Kaggle episodes):
1. tiny-need target no longer blackouts phase 1 (send floor instead of skip);
2. launch paths are checked against blocking planets, not just the sun;
3. no shots at rotating targets on turn 0 (rotation sign unknown until turn 1).
...
def _path_blocked(x0, y0, x1, y1, P, src_id, tgt_id):
    # another planet sitting on the launch line would swallow the fleet
    for pid, p in P.items():
        if pid == src_id or pid == tgt_id:
            continue
        if _seg_hits_circle(x0, y0, x1, y1, p["x"], p["y"], p["r"] + 0.6):
            return True
    return False
...
def agent(obs):
    ...
    for src in sorted(mine, key=lambda m: front_d[m["id"]]):
        ...
        for tgt in others:
            ...
            if _path_blocked(src["x"], src["y"], ax, ay, P, src["id"], tgt["id"]):
                continue
    ...
    for tgt in enemy_t:
        ...
        for s in srcs:
            ...
            if _seg_hits_sun(s["x"], s["y"], ax, ay) or eta > remaining:
                continue
            if _path_blocked(s["x"], s["y"], ax, ay, P, s["id"], tgt["id"]):
                continue
    ...
    for s in mine:
        ...
        if _seg_hits_sun(s["x"], s["y"], ax, ay) or eta > remaining:
            continue
        if _path_blocked(s["x"], s["y"], ax, ay, P, s["id"], dest["id"]):
            continue
        ang = math.atan2(ay - s["y"], ax - s["x"])
