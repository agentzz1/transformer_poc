"""Orbit Wars v6 — v5 focus-drip + early proximity weighting + contested-neutral sniping.
...
def agent(obs):
    ...
    # remaining need per neutral; contested neutrals (enemy inbound) get sniped:
    # after the enemy fleet clears the garrison, the flip costs only the surplus
    need = {}
    for t in others:
        if t["owner"] == -1:
            ein = enemy_in(t["id"])
            base = abs(t["ships"] - ein) + 2 if ein > 0 else t["ships"] + 1
            need[t["id"]] = base - friendly_in(t["id"])
    ...
    for src in sorted(mine, key=lambda m: front_d[m["id"]]):
        ...
        for tgt in others:
            ...
            if pay < rem_need:  # won't amortize before game end
                continue
            # early game: strongly prefer close, cheap captures (production base
            # first); later the horizon widens
            eta_w = 0.20 if step < 120 else 0.05
            val = (tgt["prod"] + 0.3) / (rem_need * (1.0 + eta_w * eta))
            if val > best_val:
